...
/*
    ErrorInfo AStarPlanner::SearchPath(const int &start_index,
        ...
        int current_index, move_cost, h_score, count = 0, direction_change_cost;
        ...
        while (!openlist.empty()) {
            ...
            for (auto neighbor_index : neighbors_index) {
                ...
                GetDirectionChangeCost(current_index, neighbor_index, direction_change_cost);
                

                if (g_score_.at(neighbor_index) > g_score_.at(current_index) + move_cost + direction_change_cost + cost_[neighbor_index]) {

                    g_score_.at(neighbor_index) = g_score_.at(current_index) + move_cost + direction_change_cost + cost_[neighbor_index];
...
//    }
        ...
        double calculate_time_ = std::chrono::duration<double, std::ratio<1, 1000>>(std::chrono::high_resolution_clock::now() - start).count();
        ROS_INFO("A_start time for once: %.5lf", calculate_time_);
    ...
    ErrorInfo AStarPlanner::SearchPath(const int &start_index,
        ...
        g_score_.at(start_index) = 0;
        openlist.push(std::make_pair(0, start_index));

        std::vector<int> neighbors_index;
        int current_index, move_cost, h_score, count = 0, direction_change_cost, current_cost;

        auto start = std::chrono::high_resolution_clock::now();
        while (!openlist.empty()) {
            current_index = openlist.top().second;
            current_cost = openlist.top().first;
            openlist.pop();
            state_.at(current_index) = SearchState::CLOSED;
            ...
            if(current_cost > g_score_.at(current_index)) { //the pair pushed long long ago
                continue;
            }
            GetNineNeighbors(current_index, neighbors_index);
            for (auto neighbor_index : neighbors_index) {
                ...
                if (cost_[neighbor_index] >= inaccessible_cost_ - 5 || state_.at(neighbor_index) == SearchState::CLOSED) {
                    continue;
                }
                GetMoveCost(current_index, neighbor_index, move_cost);
                GetDirectionChangeCost(current_index, neighbor_index, direction_change_cost);
                if (g_score_.at(neighbor_index) > g_score_.at(current_index) + move_cost + direction_change_cost + cost_[neighbor_index]) {
                    g_score_.at(neighbor_index) = g_score_.at(current_index) + move_cost + direction_change_cost + cost_[neighbor_index];
                    parent_.at(neighbor_index) = current_index;
                    openlist.push(std::make_pair(g_score_.at(neighbor_index), neighbor_index));
                }
            }
            count++;
        }
...
//    }
    ...
    void AStarPlanner::GetDirectionChangeCost(const int &current_index, const int &neighbor_index, int &direction_change_cost) const {
        if(parent_.at(current_index) == std::numeric_limits<int>::max()) direction_change_cost = 0;
        else{
            unsigned int pre_index = parent_.at(current_index), pre_cell_x, pre_cell_y, cur_cell_x, cur_cell_y, nxt_cell_x, nxt_cell_y;
            costmap_ptr_->GetCostMap()->Index2Cells(pre_index, pre_cell_x, pre_cell_y);
            costmap_ptr_->GetCostMap()->Index2Cells(current_index, cur_cell_x, cur_cell_y);
            costmap_ptr_->GetCostMap()->Index2Cells(neighbor_index, nxt_cell_x, nxt_cell_y);
            direction_change_cost = std::abs( (int)((cur_cell_x - pre_cell_x) * (nxt_cell_y - cur_cell_y) - (int)(cur_cell_y - pre_cell_y) * (nxt_cell_x - cur_cell_x)) ) * 2000;
            //ROS_INFO("pre - cur:(%d, %d), cur - nxt:(%d, %d), cost: %d", cur_cell_x - pre_cell_x, cur_cell_y - pre_cell_y, nxt_cell_x - cur_cell_x, nxt_cell_y - cur_cell_y, direction_change_cost);
        }
    }
