...
from actions import Patrol, MoveToGoal, Stop, ShooterAwake
...
if __name__ == "__main__":
    ...
    #     blackboard_keys={"data_pool"})
    # #顶层选择器 top_selector 会根据条件选择手动控制或自动控制行为。# 1. 创建 Stop 节点
    stop_node = Stop.Stop(name="Stop", chassis_executor=cr, data_pool=data_pool)

    # 2. 创建 EternalGuard，保证在 CheckStop 为 True 时执行 Stop 节点
    eternal_stop_node = py_trees.decorators.EternalGuard(
        name="Eternal Stop Guard",
        child=stop_node,
        condition=behavior_conditions.CheckStop,  # 停止判断条件
        blackboard_keys={"data_pool"}
    )

    # 3. 创建自动控制或手动控制的选择器
    control_selector = py_trees.composites.Selector(
        name="Control Selector",
        memory=False,
        children=[eternal_guard_manual_control, eternal_guard_auto_control]  # 自动控制和手动控制
    )

    # 4. 创建顶层 Selector，首先判断是否停止，如果没有停止，则进入自动/手动控制的判断
    top_selector = py_trees.composites.Selector(name="TopSelector", memory=False)

    # 先添加 EternalGuard Stop 节点，检查是否需要停止
    top_selector.add_children([eternal_stop_node, control_selector])

    # 5. 创建一个 EternalGuard 用来检查系统是否需要运行
    eternal_guard_system_run = py_trees.decorators.EternalGuard(
        name="Eternal Guard Top",
        child=top_selector,
        condition=behavior_conditions.CheckSystemRunCondition,  # 系统是否运行，决定是否继续行为树
        blackboard_keys={"data_pool"}
    )

    # 6. 将 EternalGuard Stop 和行为树根节点添加
    root.add_child(eternal_guard_system_run)

    
    
    gr = GimbalExecutor.GimbalExecutor()
    action_shooter_awake = ShooterAwake.ShooterAwake(
        name="ShooterAwake",
        gimbal_executor=gr,
        data_pool=data_pool
    )   
    action_shooter_awake.setup()
    eternal_shooter_awake = py_trees.decorators.EternalGuard(
        name="Eternal ShooterAwake",
        child=action_shooter_awake,
        condition=behavior_conditions.Checkshoot,  #是否巡逻的判断
        blackboard_keys={"data_pool"})
    root.add_child(eternal_shooter_awake)
