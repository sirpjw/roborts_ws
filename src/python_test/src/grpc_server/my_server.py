...
def setBlood(blood: int, data_pool_: DataPool.DataPool) -> bool:
    if blood < 0:
        ...
        data_pool_.robotinfo_variables.robot_hp = blood
        return True
...
def setAmmunition(ammunition, types, data_pool_: DataPool.DataPool):
    if types == 1:
        ...
        if ammunition < 0:
            ...
            data_pool_.robotinfo_variables.robot_bullets +=  ammunition
            return True
        ...
        new_bullet_count = data_pool_.robotinfo_variables.robot_bullets - ammunition
        if new_bullet_count < 0:
            data_pool_.robotinfo_variables.robot_bullets = 0  # 弹药不能为负
        else:
            data_pool_.robotinfo_variables.robot_bullets = new_bullet_count
        return True
        ...
        return False  # 如果type不是1、2、3，返回失败
...
def setNavigation(x, y, data_pool_: DataPool.DataPool):
    data_pool_.my_goal.pose.position.x = x
    data_pool_.my_goal.pose.position.y = y
    return True
...
def setFrictionWheel(x, data_pool_: DataPool.DataPool):
    try:
        if x == 0:
            # 设置摩擦轮停止
            data_pool_.robotinfo_variables.fricwheel_enable = False
        elif x == 1:
            # 设置摩擦轮启用
            data_pool_.robotinfo_variables.fricwheel_enable = True
        else:
...
def setStop(x, data_pool_: DataPool.DataPool):
    try:
        if x == 0:
            # 设置底盘停止
            data_pool_.robotinfo_variables.chassis_enable = False
        elif x == 1:
            # 设置底盘启用
            data_pool_.robotinfo_variables.chassis_enable = True
        else:
            rospy.logwarn("Invalid value for x, should be 0 or 1.")
            return False
...
def setBullet(x, data_pool_: DataPool.DataPool):
    try:
        if x == 0:
            # 设置射击停止
            data_pool_.robotinfo_variables.shooter_enable = False
        elif x == 1:
            # 设置射击启用
            data_pool_.robotinfo_variables.shooter_enable = True
        else:
            rospy.logwarn("Invalid value for x, should be 0 or 1.")
            return False
