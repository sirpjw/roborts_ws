...
namespace roborts_detection {
    ...
    void ArmorDetection::LoadParam(cv::Mat intrinsic_matrix, cv::Mat distortion_coeffs) {
        ros::NodeHandle ros_nh_;
        ros_nh_.param("enemy_color", enemy_color_, false);
        ros_nh_.param("enable_debug", enable_debug_, true);
        ros_nh_.param("light_min_angle", light_min_angle_, 60.0);
        light_max_angle_ = 180 - light_min_angle_;
        ros_nh_.param("light_max_angle_diff", light_max_angle_diff_, 20.0);
        ros_nh_.param("light_aspect_ratio", light_aspect_ratio_, 1.5);
        ros_nh_.param("armor_max_angle", armor_max_angle_, 25.0);
        ros_nh_.param("armor_max_aspect_ratio", armor_max_aspect_ratio_, 3.0);
        ros_nh_.param("armor_min_aspect_ratio", armor_min_aspect_ratio_, 1.2);
        ros_nh_.param("blue_threshold", blue_threshold_, 100);
        ros_nh_.param("red_threshold", red_threshold_, 50);
        ros_nh_.param("red_brightness_threshold", red_brightness_threshold_, 55);
        ros_nh_.param("blue_brightness_threshold", blue_brightness_threshold_, 100);
    ...
    void ArmorDetection::DetectArmor(cv::Mat &src_img_, bool &detected, cv::Point3f &target_3d, cv::Point3f &target_r) {
        ...
        if (!armors.empty()) {
            ...
            double time_ = (ros::Time::now() - pre).toSec();
            int FPS = (int)(1.0 / time_);

            //ROS_INFO("final_armor_x: %.4lf final_armor_y: %.4lf", final_armor_.center.x, final_armor_.center.y);
            ...
            //ss << "Position: x = " <<(int)target_3d.x << ", y = " << (int)target_3d.y << ", z = " << (int)target_3d.z;
            std::string res = (std::string)("Armor Position: x = ") + std::to_string((int)target_3d.x) + (std::string)(", y = ") + std::to_string((int)target_3d.y) + (std::string)(", z = ") + std::to_string((int)target_3d.z);
            cv::putText(src_img_, (std::string)("Frames Per Sec: ") + std::to_string(FPS), cv::Point(0,20), cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0,0,255), 1);
            cv::putText(src_img_, res , cv::Point(0,40), cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0,0,255), 1);
        } else {
            detected = false;
        }
        if(enable_debug_) {
    ...
    void ArmorDetection::DetectLights(const cv::Mat &src_img_, std::vector<cv::RotatedRect> &lights, std::vector<double> &lights_angle) {
        ...
        for (int i = 0; i < contours_brightness.size(); ++i) {
            ...
            //ROS_INFO("armor_angle: %.1lf light_aspect_ratio: %.1lf", armor_angle, light_aspect_ratio);
    ...
    cv::RotatedRect ArmorDetection::SelectFinalArmor(std::vector<cv::RotatedRect> &armors, cv::Point3f &target_3d_) {
        ...
        for (int i = 0; i < armors.size(); i++) {
            ...
            //ROS_INFO("armor_angle: %.1lf", armor_angle);
            if(armor_angle >= 90) {
