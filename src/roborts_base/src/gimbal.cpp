...
void Gimbal::ROS_Init(){
  ...
  //ros publisher
  base_gimbal_angle_pub = ros_nh_.advertise<std_msgs::Float64>("base_gimbal_angle", 1);
  //ros_message_init
...
void Gimbal::GimbalInfoCallback(const std::shared_ptr<roborts_sdk::cmd_gimbal_info> gimbal_info){
  ...
  angle.data = gimbal_info -> yaw_ecd_angle / 1800.0 * M_PI;
  base_gimbal_angle_pub.publish(angle);
...
bool Gimbal::CtrlFricWheelService(roborts_msgs::FricWhl::Request &req,
  ...
  if(req.open){
    fric_speed.left = 1280; //1240
    fric_speed.right = 1280; //1240
  } else{
