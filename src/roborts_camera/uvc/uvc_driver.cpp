...
void UVCDriver::StartReadCamera(cv::Mat &img) {
  if(!camera_initialized_){
    ...
    SetCameraResolution(camera_info_.camera_path);
...
void UVCDriver::SetCameraResolution(std::string id) {
  int cam_fd;
  if ((cam_fd = open(id.c_str(), O_RDWR)) == -1) {
    std::cerr << "Camera open error" << std::endl;
  }
  ...
  fmt.fmt.pix.width = camera_info_.resolution_width;
  fmt.fmt.pix.height = camera_info_.resolution_height;
  fmt.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
  if(ioctl(cam_fd, VIDIOC_G_FMT, &fmt) < 0) {
    std::cerr << "Set Camera Resolution Error" << std::endl;
  }
